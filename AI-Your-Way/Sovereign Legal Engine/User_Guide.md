...
## 10. Remote Access — Hosting Live Client Demos

### Option 1 — ngrok *(Recommended for demos)*

Encrypted HTTPS tunnel from a public URL to your local port 8001. No port forwarding, no router config.

```bash
# Install (Windows)
choco install ngrok

# Start engine first, then:
ngrok http --basic-auth="demo:yourpassword" 8001
# Output: Forwarding https://a1b2c3d4.ngrok-free.app -> localhost:8001
```

- Free tier — URL changes each session. Free account lets you pin a static subdomain.
- Share the `https://` URL with your client. TLS-encrypted end-to-end, no client install required.

### Option 2 — Tailscale *(Best for recurring client access)*

Private WireGuard VPN mesh. Client installs the free Tailscale app, you invite them to your tailnet, they access the engine at your Tailscale IP. No public internet exposure.

```bash
# 1. Install Tailscale: tailscale.com/download
# 2. Invite client from tailscale.com/admin
# 3. Client installs app and accepts invite
# 4. Get your Tailscale IP:
tailscale ip -4
# Client browses to: http://100.x.x.x:8001
```

- Access revocable instantly from the admin panel
- Works through NAT, firewalls, hotel/corporate networks
- Free tier: up to 3 users

### Option 3 — Port Forwarding *(Controlled LAN only)*

```powershell
# Open firewall port (run as Admin):
netsh advfirewall firewall add rule name="LegalEngine" protocol=TCP dir=in localport=8001 action=allow

# Get your public IP:
curl ifconfig.me
# Then forward port 8001 to your laptop's LAN IP in your router.
# Client URL: http://<your-public-ip>:8001
```

> **WARNING:** Plain HTTP, no built-in auth, direct internet exposure. Only use for short demos on a network you control. Remove the firewall rule when done.

### Comparison

| | ngrok | Tailscale | Port Forward |
|---|---|---|---|
| **Setup time** | 2 min | 10 min | 15–30 min |
| **HTTPS** | Yes (auto) | No (encrypted tunnel) | No |
| **Auth** | Basic auth flag | Tailscale account | None built-in |
| **Client install** | Not required | Required (free app) | Not required |
| **Works through NAT** | Yes | Yes | Requires port forward |
| **Best for** | One-off demos | Recurring client access | Controlled LAN only |
