...
### Why I Built It
In my experience, legal documents are a nightmare for standard AI. Scanned PDFs, broken table structures, and complex formatting usually lead to garbled noise or, worse, hallucinations. I didn't want to build a tool that "guessed" at findings. I wanted an engine that verified the data before it ever hit the model.
...
I realized that asking one small model to do everything causes "context drift." To solve this, I split the "brain" into two specialized agents running in parallel on my local CPU. As always, I'm using small pre-trained models on a laptop to bring AI and LLMs to the real world without having to waste money on subscriptions and big GPU-based hardware:
...
While I've included a few "pre-made" defaults for things like SCOTUS audits, the real power is that the user can apply and create their own custom prompts to suit the specific legal matter at hand. Whether I'm doing a deposition summary or a contract review, I just commit the new logic and the engine adapts.
